...
'''
    Reinforcement learning environment for traffic light agent, this is environment 2
'''
...
class TrafficLight:
    ...
    def computeReward(self, action):
        ...
        return self.chooseMinWaitTime(action, update_reward) + self.conn.simulation.getArrivedNumber()
    ...
    def getObservation(self):
        ...
        for i in range (len(self.lanes_id)):
            if np.isnan((density[i]+queue[i])/2) :
                obs.append(0.0)
                print(1)
            else:
